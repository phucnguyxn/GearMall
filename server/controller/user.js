...
const register = asyncHandler(async (req, res) => {
    ...
    if (user) throw new Error('User has existed')
        ...
        return res.status(200).json({
            ...
            mes: newUser ? 'Registed is successfully. Please login now~' : 'Something went wrong'
        })
    }
})
//Check login
const login = asyncHandler(async (req, res) => {
    ...
    if (response && await response.isCorrectPassword(password)) {
        //Tach pw va role khoi response
        const { password, role, refreshToken, ...userData } = response.toObject()
        //Tao accessToken
        const accessToken = generateAccessToken(response._id, role)
        //Tao refreshToken
        const newRefreshToken = generateRefreshToken(response._id)
        //Luu refreshToken vao dtb
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    if (!cookie && !cookie.refreshToken) throw Error('No refresh token in cookie')
    //Check token hop le
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    const html = `Vui lòng click vào link dưới đây để đổi mật khẩu .Link sẽ hết hạn sau 15 phút . <a href=${process.env.URL_SERVER}/api/user/reset-password/${resetToken}>Click here</a>`
